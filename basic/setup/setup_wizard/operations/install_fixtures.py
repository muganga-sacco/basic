...
# from basic.accounts.doctype.account.account import RootNotEditable
# from basic.regional.address_template.setup import set_up_address_templates
...
def install(country=None):
    ...
    for doctype, title_field, filename in (
        ...
        records += [
            {"doctype": doctype, title_field: title} for title in read_lines(filename)
        ]
    ...
    from basic import make_default_records
...
def install_defaults(args=None):  # nosemgrep
    ...
    frappe.db.set_single_value(
        "Stock Settings", "email_footer_address", args.get("company_name")
    )
...
def create_bank_account(args):
    ...
    if bank_account_group:
        ...
        try:
            ...
            frappe.throw(
                _("Bank account cannot be named as {0}").format(

...
from basic.setup.setup_wizard.operations.taxes_setup import setup_taxes_and_charges
from basic.utils import get_account_currency
...
class Company(NestedSet):
    ...
    if TYPE_CHECKING:
        ...
        country: DF.Link
        create_chart_of_accounts_based_on: DF.Literal[
    ...
    def on_update(self):
        ...
        if (
            ...
            frappe.local.enable_perpetual_inventory[self.name] = (
    ...
    def on_trash(self):
        ...
        frappe.db.sql(
            "delete from `tabTax Withholding Account` where company=%s", self.name
        )
        frappe.db.sql(
            "delete from `tabTransaction Deletion Record` where company=%s", self.name
        )
...
def install_country_fixtures(company, country):
    try:
        module_name = f"basic.regional.{frappe.scrub(country)}.setup.setup"
...
def get_default_company_address(
    name, sort_key="is_primary_address", existing_address=None
):

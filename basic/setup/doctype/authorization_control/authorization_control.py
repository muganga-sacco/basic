...
from basic.utils.transaction_base import TransactionBase
...
class AuthorizationControl(TransactionBase):
    ...
    def get_appr_user_role(
        ...
        if det:
            ...
            if not has_common(appr_roles, frappe.get_roles()) and not has_common(
                ...
                frappe.msgprint(
                    _("Not authroized since {0} exceeds limits").format(_(based_on))
                )
                frappe.throw(
                    _("Can be approved by {0}").format(
                        comma_or(appr_roles + appr_users)
                    )
                )

    def validate_auth_rule(
        self, doctype_name, total, based_on, cond, company, master_name=""
    ):
        chk = 1
        add_cond1, add_cond2 = "", ""
    ...
    def bifurcate_based_on_type(
        ...
        if based_on == "Itemwise Discount":
            ...
            self.validate_auth_rule(
                doctype_name, auth_value, based_on, add_cond, company
            )
    ...
    def validate_approving_authority(self, doctype_name, company, total, doc_obj=""):
        ...
        if doc_obj:
            ...
            for d in doc_obj.get("items"):
                if d.base_rate:
                    price_list_rate += (
                        flt(d.base_price_list_rate) or flt(d.base_rate)
                    ) * flt(d.qty)
        ...
        for r in based_on:
            if r in final_based_on and not r in [
                "Itemwise Discount",
                "Item Group wise Discount",
            ]:
    ...
    def get_value_based_rule(
        self, doctype_name, employee, total_claimed_amount, company
    ):
        val_lst = []
        val = frappe.db.sql(
            """select value from `tabAuthorization Rule`

...
class HolidayList(Document):
    ...
    if TYPE_CHECKING:
        ...
        from basic.setup.doctype.holiday.holiday import Holiday
    ...
    def get_weekly_off_dates(self):
        ...
        for d in self.get_weekly_off_date_list(self.from_date, self.to_date):
            ...
            self.append(
                "holidays",
                {"description": _(self.weekly_off), "holiday_date": d, "weekly_off": 1},
            )
...
def is_holiday(holiday_list, date=None):
    ...
    if holiday_list:
        return bool(
            frappe.db.exists(
                "Holiday", {"parent": holiday_list, "holiday_date": date}, cache=True
            )
        )
    else:
        return False
...
def local_country_name(country_code: str) -> str:
    ...
    return Locale.parse(frappe.local.lang, sep="-").territories.get(
